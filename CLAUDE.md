...
| View BOQ finalize — "QS project type must be configured" | `ManualSetupShell.tsx`, `ProjectWorkspaceView.tsx` | **Fixed 2026-09-07.** Root cause: 2-step wizard never called `PATCH /takeoff/:id/qs-config`, so `qsProjectType` was unset and the backend `finalize` 400'd. Fix: Step 1 now has a required "Scope of Works" dropdown (`SCOPE_PROJECT_TYPES`); `ManualSetupShell` calls `updateQsConfig` right after `createProject` via `buildQsConfigPayloadFromScope()`. Projects created before the fix are recovered by a scope-picker `Dialog` in `handleViewBoq` that PATCHes qs-config then retries finalize. `handleViewBoq` also now surfaces the real backend `message` on 400 instead of the canned "ensure at least one measurement is saved". **2026-09-08:** removed the "No active session" blocking toast — `handleViewBoq` now calls `resolveBoqSessionId()` (active page session → project's active session → any non-finalized session → any session), and if the project has zero sessions it just routes to `/boq` and lets that page render its own empty state. Resolved id is stashed in `pendingBoqSessionId` ref so `submitScopeAndFinalize` uses it too. |
...
| Element list "delete doesn't stick" | `ProjectWorkspaceView.tsx` `handleCreateNewEl` | **Fixed 2026-09-08.** `newEl.id` was `""` for every "Create New Element" — the comment assumed the backend owns element identity, but the backend keys by the FE-sent `${id}-${tool}` clientId and stores `attributes.elementId` verbatim. Blank id → all created elements shared clientId `-${tool}` server-side (last write wins), duplicate React `key=""` (list DOM wouldn't re-render on delete), `elements.filter(e => e.id !== el.id)` removed all-or-nothing, and `loadProjectElements` skipped them (`if (!eid) continue`). Fix: `id: crypto.randomUUID()`. Elements created before the fix have `elementId: ""` server-side; a **one-time migration effect** in `ProjectWorkspaceView.tsx` (guarded by `localStorage["qp:orphan-elements-cleaned:<projectId>"]`) DELETEs the bare `-count`/`-length`/`-area` clientIds for every project session on first workspace load. Safe to delete that block once all active projects have opened the workspace once. |
...
| `RowEditPanel.tsx` | **Floating draggable card** (was `RowEditSheet`, a right `Sheet`, until 2026-09-08) — edit description/leadIn/unit/qty/rate/itemCode + unlock toggles + live `qty × rate` amount readout. Opens top-centre, drag by the header, Esc/✕ to close, collapse toggle. Built on the reusable `components/ui/draggable-panel.tsx` + `hooks/useDraggablePanel.ts` (viewport-clamped pointer drag, re-clamps on resize). Section context (`sectionCode`/`sectionTitle`) is threaded through `onEditRow(row, section)` for the panel subtitle. The workspace's `ElementDetailPanel` has its own older inline drag logic and could later adopt the same hook. |
