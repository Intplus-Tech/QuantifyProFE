  ...
  return (
    <div className="border border-slate-200 rounded-xl bg-white shadow-sm overflow-hidden flex flex-col">
      <div className="overflow-x-auto w-full">
        <table className="w-full text-sm text-left border-collapse">
          ...
          <thead className="bg-[#fcfdfd] border-b border-slate-200">
            <tr>
              <th className="px-6 py-4 text-xs font-bold text-slate-800 uppercase tracking-wider w-20">
                Item
              </th>
              <th className="px-6 py-4 text-xs font-bold text-slate-800 uppercase tracking-wider">
                Description
              </th>
              <th className="px-6 py-4 text-center text-xs font-bold text-slate-800 uppercase tracking-wider w-24">
                Unit
              </th>
              <th className="px-6 py-4 text-center text-xs font-bold text-slate-800 uppercase tracking-wider w-24">
                Qty
              </th>
              <th className="px-6 py-4 text-center text-xs font-bold text-slate-800 uppercase tracking-wider w-32">
                Rate ($)
              </th>
              <th className="px-6 py-4 text-right text-xs font-bold text-slate-800 uppercase tracking-wider w-40">
                Total ($)
              </th>
            </tr>
          </thead>
      ...
      <div className="bg-amber-500 p-5 px-6! flex justify-between items-center text-white">
        <span className="font-bold flex-1 text-right  uppercase tracking-wider text-xl">
  ...
  return (
    <>
      ...
      {section.items.map((item) => {
        ...
        return (
          ...
          >
            ...
            <td
              className={`px-6 py-4 text-slate-700 ${isHeader ? "font-bold text-slate-900" : ""}`}
            >
              {item.description}
            </td>
      ...
      <tr className="border-b border-slate-200">
        ...
        >
          Sub-total{" "}
          {section.title.split("-")[1]?.trim() || section.title.split(" ")[1]}:
        </td>
