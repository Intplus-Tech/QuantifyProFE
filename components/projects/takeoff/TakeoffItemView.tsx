...
export function TakeoffItemView({
  ...
  const [bendingSummaries, setBendingSummaries] = useState<
        ...
        tab.subTabs.forEach((sub) => {
          if (sub.tables) {
            sub.tables.forEach((table) => {
              initialRows[`${tab.id}-${sub.id}-${table.id}`] = [
                ...table.defaultRows,
              ];
            });
  ...
  const addRow = (tabId: string, customPrefix?: string) => {
    setTabRows((prev) => {
      ...
      const prefix =
        customPrefix ||
        (newTabRows.length > 0 && newTabRows[0].id
          ? newTabRows[0].id.replace(/[0-9]/g, "")
          : "ROW");
                    ...
                    <Button
                      ...
                      className={
                        bendingSummaries[rowKey]
                          ? "bg-slate-900 text-white hover:bg-slate-800 hover:text-white h-9 font-medium border-slate-900 shadow-sm"
                          : "text-slate-600 bg-white shadow-sm border-slate-200 hover:bg-slate-50 h-9 font-medium"
                      }
                      onClick={() => {
                        setBendingFormData(
                          bendingSummaries[rowKey] || {
                    ...
                    <tbody className="divide-y divide-slate-100">
                      <tr className="hover:bg-slate-50/40 transition-colors">
                        <td className="px-4 py-4">
                          {bendingSummaries[rowKey].Y6 || "0"}
                        </td>
                        <td className="px-4 py-4">
                          {bendingSummaries[rowKey].Y8 || "0"}
                        </td>
                        <td className="px-4 py-4">
                          {bendingSummaries[rowKey].Y10 || "0"}
                        </td>
                        <td className="px-4 py-4">
                          {bendingSummaries[rowKey].Y12 || "0"}
                        </td>
                        <td className="px-4 py-4">
                          {bendingSummaries[rowKey].Y16 || "0"}
                        </td>
                        <td className="px-4 py-4">
                          {bendingSummaries[rowKey].Y20 || "0"}
                        </td>
                        <td className="px-4 py-4">
                          {bendingSummaries[rowKey].Y25 || "0"}
                        </td>
                          ...
                          {(() => {
                            const sum = [
                              ...
                              const val = parseFloat(
                                bendingSummaries[rowKey][key]?.replace(
                                  /,/g,
                                  "",
                                ) || "0",
                              );
                        ...
                        <td className="px-4 py-4 font-bold text-slate-800">
                          {(() => {
                            ...
                            return (sum / 1000).toLocaleString(undefined, {
                  ...
                  return (
                    <div className="p-8 text-center border rounded-xl bg-white border-slate-200">
                      <p className="text-slate-500">
                        Please add rows to the{" "}
                        {tableData.groupedBy.replace(/-/g, " ")} tab first.
                      </p>
                    </div>
                  );
                }
                ...
                return (
                  <div className="space-y-6">
                    {sourceRows.map((sourceRow, idx) => {
                      const groupKey =
                        sourceRow.id ||
                        `${tableData.groupIdPrefix || "G"}${idx + 1}`;
